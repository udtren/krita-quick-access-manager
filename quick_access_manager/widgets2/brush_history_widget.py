...
class BrushHistoryWidget(QWidget):
    ...
    def __init__(self, parent=None, brush_history_number=20, icon_size=30):
        ...
        self.BRUSHES_PER_ROW = brush_history_number // 2  # Brushes per row (half of total)
        self.ICON_SIZE = icon_size
        self.brush_history = []
        self.brush_buttons = []
        self.init_ui()
        
        print(f"BrushHistoryWidget initialized with {self.BRUSHES_PER_ROW} brushes per row (2 rows, {self.TOTAL_BRUSHES} total), icon size: {icon_size}")  # Debug output
    ...
    def init_ui(self):
        ...
        for i in range(self.TOTAL_BRUSHES):
            ...
            brush_btn.setIconSize(QSize(button_size - 4, button_size - 4))  # Icon slightly smaller than button
            brush_btn.setStyleSheet("border: 1px solid #888; background-color: #f0f0f0;")
        ...
        try:
            ...
            if brush_image:
                ...
                if not pixmap.isNull():
                    ...
                    scaled_pixmap = pixmap.scaled(size, size, 1, 1)  # Qt.KeepAspectRatio, Qt.SmoothTransformation
            ...
            try:
                ...
                if current_preset:
                    ...
                    if not self.brush_history or self.brush_history[0][0] != brush_name:
                        print(f"Brush changed, adding to history: {brush_name}")  # Debug output
    ...
    def add_brush_to_history(self, brush_name, brush_preset):
        ...
        for i, (name, preset) in enumerate(self.brush_history):
            if name == brush_name:
                print(f"Removing existing brush from position {i}: {name}")  # Debug output
        ...
        if len(self.brush_history) > self.TOTAL_BRUSHES:
            ...
            print(f"Trimmed history, removed: {[b[0] for b in removed]}")  # Debug output
        
        # Update button display
        self.update_brush_buttons()
        print(f"Brush history now has {len(self.brush_history)} brushes: {[b[0] for b in self.brush_history]}")  # Debug output
    
    def update_brush_buttons(self):
        """Update the brush buttons to show current history"""
        print(f"Updating brush buttons with {len(self.brush_history)} brushes")  # Debug output
