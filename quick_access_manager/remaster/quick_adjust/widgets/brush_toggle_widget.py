...
bottom section of the HueSVC popup's right panel.

Reads/writes the current brush preset's XML directly (same technique as the
legacy CompactBrushToggler plugin): pull the preset XML via Preset.toXML(),
flip the relevant <param> element's text between "true"/"false", write it
back via preset.fromXML(). PressureSize and PressureRotation each pair with a
...
from ...compat import QGridLayout, QPushButton, QWidget
from ..settings import get_font_size
