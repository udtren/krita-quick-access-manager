...
class ShortcutButtonConfigDialog(QDialog):
    ...
    def load_current_values(self):
        ...
        font_size = config.get("fontSize") if config else str(max(self.button.font().pointSize(), 7))
        if not font_size or font_size == "-1":
            font_size = str(max(self.button.font().pointSize(), 7))
        self.font_size_edit.setText(str(font_size))
        
        # Colors
        self.bg_color_edit.setText(self.button.palette().color(self.button.backgroundRole()).name())
        self.font_color_edit.setText(self.button.palette().color(self.button.foregroundRole()).name())